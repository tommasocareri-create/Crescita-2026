"""
reader.py — Legge e struttura i dati dalla pagina C.E. scaricando da Google Sheets.
"""
import openpyxl
...
# Usa il tuo link "Pubblica sul Web", ma forzando l'output nativo in Excel (xlsx)
EXCEL_URL = "https://docs.google.com/spreadsheets/d/e/2PACX-1vQJaIzdbR_Bs3r2nVZWPWglma3Vnd4Lp2m6yZYnBsJk3kQYBqcqS51yHBA1VapuXgjDk6j3kphwpsED/pub?output=xlsx"
SHEET_NAME = "C.E."
...
def load_data(path=None):
    ...
    # Se Google ci blocca restituendo una pagina web HTML invece del file Excel, ce ne accorgiamo
    if b"<html" in resp.content.lower():
        raise Exception("Blocco di sicurezza di Google rilevato. Verifica il link Pubblica sul Web.")

    # Carica il workbook usando la memoria RAM (BytesIO)
    wb = openpyxl.load_workbook(filename=BytesIO(resp.content), data_only=True)
    if SHEET_NAME not in wb.sheetnames:
        raise Exception(f"Il foglio '{SHEET_NAME}' non esiste nel file.")
    ws = wb[SHEET_NAME]
    ...
    for col_letter, month_name in MONTH_MAP:
        ...
        for i, a in enumerate(assets):
            ...
            asset_data.append({"name": a["name"], "value": v, "var_eur": var, "var_pct": vp, "weight": w})
    ...
    for col_letter, month_name in MONTH_MAP:
        ...
        monthly_income[month_name] = {
            "stipendio": stipendio, "altre": altre, "lorde": lorde, "nette": nette,
    ...
    for row, label in INCOME_ROWS.items():
        avg   = _safe(ws.cell(row=row, column=54).value)
        total = _safe(ws.cell(row=row, column=55).value)
        yoy   = _safe(ws.cell(row=row, column=56).value)
