...
:root {--navy:#0D1B2A;--navy2:#0A1520;--gold:#F0C040;--gold2:#FFD966;--teal:#00E5CC;--red:#FF5C5C;--cream:#FFFFFF;--muted:#B0C4D8;--card:#152233;--border:#1E3448;--green:#00E676;}
...
.pos{color:#00E676!important;font-weight:600;}.neg{color:#FF5C5C!important;font-weight:600;}.neu{color:var(--muted);}.teal{color:var(--teal)!important;font-weight:600;}
.prog-wrap{background:var(--border);border-radius:4px;height:7px;margin-top:7px;}.prog-bar{height:7px;border-radius:4px;}
...
def kpi(label, value, sub="", sub_class="neu", gold=False):
    ...
    return f'<div class="kpi"><div class="kpi-lbl">{label}</div><div class="kpi-val" style="color:{val_col}">{value}</div><div class="kpi-sub {sub_class}">{sub}</div></div>'

def kpi_stip(label, value, sub=""):
    return f'<div class="kpi-stip"><div class="kpi-lbl">{label}</div><div class="kpi-val" style="color:#FFD966">{value}</div><div class="kpi-sub" style="color:#F0C040">{sub}</div></div>'

CHART_LAYOUT = dict(plot_bgcolor="rgba(0,0,0,0)", paper_bgcolor="rgba(0,0,0,0)", font=dict(color="#FFFFFF", family="DM Sans"), margin=dict(l=10, r=10, t=20, b=10))
...
# ── Carica dati PRIMA della sidebar ──────────────────────────────────────────
try:
    data = get_data()
...
with st.sidebar:
    ...
    st.divider()
    page = st.radio("Navigazione", ["📊 Dashboard", "📋 Tabella Mensile", "📈 Grafici", "💸 Entrate"], label_visibility="collapsed")
    st.divider()
    ...
    pills = "".join(f'<span class="pill {"pill-ok" if m in filled else "pill-no"}">{m[:3].upper()}</span>' for m in months_order)
...
if page == "📊 Dashboard":
    ...
    with k1:
        st.markdown(
            kpi(f"Patrimonio Attuale {sym}", fe(last_total, mul, sym), f"{fe(delta_ytd, mul, sym, sign=True)} ({fp(pct_ytd)}) da dic 2025", dc(delta_ytd))
    ...
    with k3:
        ...
        st.markdown(kpi("Variazione Mensile", fp(pct_mom) if last_month else "—", f"{fe(delta_mom, mul, sym, sign=True)} · {lbl}", dc(delta_mom)), unsafe_allow_html=True)
    with k4:
        ann = ((1 + cagr_ytd) ** (12 / months_done) - 1) if months_done > 0 and cagr_ytd != 0 else 0
        st.markdown(kpi("CAGR Annualizzato", fp(ann), f"YTD: {fp(cagr_ytd)} · {months_done} mes{'e' if months_done==1 else 'i'}", dc(ann)), unsafe_allow_html=True)
    ...
    if filled:
        ...
        fig = go.Figure()
        fig.add_trace(go.Scatter(x=x, y=totals_g, name="Patrimonio", mode="lines+markers",
            line=dict(color="#F0C040", width=2.5), marker=dict(size=8, color="#F0C040"),
    ...
    yax = dict(gridcolor="#1E3448",tickprefix=sym,tickformat=",.0f",color="#FFFFFF") if view=="Valore" \
        else dict(gridcolor="#1E3448",ticksuffix="%",tickformat=".1f",color="#FFFFFF")
    fig2.update_layout(height=420, legend=dict(orientation="h",y=-0.22,font_size=11,font_color="#FFFFFF"),
        xaxis=dict(gridcolor="#1E3448",color="#FFFFFF"), yaxis=yax, **CHART_LAYOUT)
    ...
    fig_hm.update_layout(height=max(300,len(assets)*30),
        xaxis=dict(color="#FFFFFF"), yaxis=dict(tickfont=dict(size=11,color="#FFFFFF")), **CHART_LAYOUT)
