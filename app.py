...
# Grafico
    ...
    if filled:
        ...
        fig.add_trace(go.Scatter(x=x, y=fc_g, name="Forecast",
            ...
            hovertemplate=f"Forecast: {sym}%{{y:,.0f}}<extra></extra>"))
...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    with k1:
        st.markdown(kpi_stip(
            "⭐ Entrate Lorde Stipendio YTD",
            f"{sym}{stip_ytd:,.0f}".replace(",","."),
            f"Media mensile: {sym}{stip_avg:,.0f}".replace(",",".")
        ), unsafe_allow_html=True)
    with k2:
...
# ── KPI ANNO PRECEDENTE ──
    ...
    with p1:
        st.markdown(kpi_stip(
            "⭐ Stip. Lordo/mese 2025",
            f"{sym}{py_stip_m:,.0f}".replace(",","."),
            f"Totale anno: {sym}{py_stip_m*12:,.0f}".replace(",",".")
        ), unsafe_allow_html=True)
    with p2:
